...
methods = [
    ...
    {"name": "VI-Stereo-DSO", "folder": "MavEurocFormat", "file": "mav0/nt_{dataset}.txt", "log_file": "mav0/VIDSO_ASL_log.csv"},
    {"name": "OKVIS2", "folder": "MavEurocFormat", "file": "mav0/tum_okvis2.txt", "log_file": "mav0/OKVIS2_ASL_log.csv"},
...
# ---------- Output CSV ----------
output_csv = r"C:\Users\jeppe\Desktop\Bags\DataConversionScripts\evo_ape_all_results_trim.csv"
...
for dataset_name in datasets:
    ...
    for method in methods:
        ...
        if method["folder"] == "MavEurocFormat" and "{dataset}" in method["file"]:
            original_file = method["file"].replace("{dataset}", dataset_name)
            traj_file = os.path.join(r"C:\Users\jeppe\Desktop\Bags\MavEurocFormat", dataset_name, original_file.replace(".txt", "_trim.txt"))
        else:
            traj_file = os.path.join(r"C:\Users\jeppe\Desktop\Bags", method["folder"], dataset_name, method["file"].replace(".txt", "_trim.txt"))

        if not os.path.isfile(traj_file):
            print(f"❌ Skipping missing trimmed trajectory: {traj_file}")
            continue
        ...
        try:
            ...
            # Match corresponding log file (same as original, not "_trim")
            log_file = os.path.join(
                r"C:\Users\jeppe\Desktop\Bags", method["folder"], dataset_name, method["log_file"]
            )
            ...
            print(f"✅ Trimmed result added to {output_csv}")
