...
# SLAM executable
SLAM_CMD="ros2 run orbslam3 mono-inertial3 $VOCAB $YAML --ros-args --remap camera:=/cam0/image_raw --remap imu:=/imu0"
...
for folder in "$BASE_DIR"/*/; do
    ...
    LOG_PATH="$folder/ORBSLAM3_3_log.csv"

    if [ -f "$folder/OrbSlam3TUM_mono3.txt" ]; then
    ...
    if [ -f "OrbSlam3TUM.txt" ]; then
        mv "OrbSlam3TUM.txt" "$folder/OrbSlam3TUM_mono3.txt"
    fi

    if [ -f "$LOG_DIR/ORBSLAM3_3_log.csv" ]; then
        mv "$LOG_DIR/ORBSLAM3_3_log.csv" "$folder/ORBSLAM3_log_mono3.csv"
    fi
done
