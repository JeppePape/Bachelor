...
CONFIG=/mnt/hgfs/Bags/config_files/okvis2_audrone.yaml
DATASET_ROOT=/mnt/hgfs/Bags/audrone
...
for folder in "$DATASET_ROOT"/*; do
  if [[ -d "$folder/mav0" ]]; then
    ...
    while true; do
      ...
      if grep -q "Progress: 99%" "$folder/mav0/okvis2_output.log" && [ "$PROGRESS_99_DETECTED" = false ]; then
        echo "⌛ Detected 99% progress — waiting 5s before assuming completion"
        PROGRESS_99_DETECTED=true
        sleep 5
        echo "✅ OKVIS2 likely completed (99% fallback): $folder"
        break
      fi
